// $Id: ConstituentSubtractor.cc 1141 2018-06-28 12:29:14Z berta $
//
...
LimitedWarning ConstituentSubtractor::_warning_unused_rhom;
  ...
  void ConstituentSubtractor::initialize(){
    if (_max_eta<=0) throw Error("SequentialConstituentSubtractor::initialize() The value for eta cut was not set or it is negative. It needs to be set before using the function initialize");
  ...
  std::vector<fastjet::PseudoJet>  ConstituentSubtractor::subtract_event(std::vector<fastjet::PseudoJet> const &particles){
    ...
    for (unsigned int i=0;i<particles.size();++i){
      if (fabs(particles[i].eta())<_max_eta) selected_particles.push_back(particles[i]);
    }
